...
static void NOINLINE JL_NORETURN task_wrapper()
{
    ...
    /* switch to the root task */
    jl_task_t *rtask = ptls->root_task;
    rtask->result = jl_nothing;
    jl_switchto(&rtask);
...
int partr_spawn(jl_task_t **t, jl_value_t *_args, int8_t sticky, int8_t detach)
{
    ...
    *t = detach ? NULL : task;
    ...
    if (!(ptls->current_task->settings & TASK_IS_STICKY))
        // TODO. yield(ptls->current_task->ctx);
        ;
...
int partr_sync(void **r, partr_t t)
{
    ...
    if (task->state != done_sym  &&  task->state != failed_sym) {
        ...
        if (task->state != done_sym  &&  task->state != failed_sym) {
            ...
            JL_UNLOCK(&task->cq.lock);
            /* yield point */
            // TODO. yield_value(ptls->current_task->ctx, (void *)yield_from_sync);
        }
