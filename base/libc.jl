...
if Sys.iswindows()
    ...
    WindowsRawSocket(handle::Ptr{Void}) = bitcast(WindowsRawSocket, handle)
    WindowsRawSocket(handle::WindowsRawSocket) = handle

    Base.cconvert(::Type{Ptr{Void}}, fd::WindowsRawSocket) = bitcast(Ptr{Void}, fd)
    ...
    function dup(src::WindowsRawSocket)
        ...
        status = ccall(:DuplicateHandle, stdcall, Int32,
            (Ptr{Void}, WindowsRawSocket, Ptr{Void}, Ptr{WindowsRawSocket}, UInt32, Int32, UInt32),
