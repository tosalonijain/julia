...
1. **Function argument**.
   This permits the use of `do` blocks for passing multiline anonymous functions.
...
4. **Type**.
   ...
   In [`read(io, String)`](@ref read), the `IO` argument appears before the type.
...
9. **Varargs**.
   ...
   `Tuple`, e.g. `Vector{T}(uninitialized, (1,2))`, or as `Varargs`, e.g.
   `Vector{T}(uninitialized, 1, 2)`.
